...
if($_POST['page'])
{
	...
	$msg = '<div class="col-xs-12 bg-line"><h2>SELFIE PHOTOS</h2></div>';

	if($result_pag_data) {

		foreach($result_pag_data  as $row) {
			$msg .= '<div class="col-xs-12 col-sm-4 col-lg-4"><a href="'.c_site_url().'user_public_profile/show_selfphoto_record/'.$uid.'/'.$row->id.'" class="thumbnail">'.$row->photo_title.'<img src="'.c_site_url().'/uploads/'.$uid.'/self/'.$row->photo_img.'" alt="'.$row->photo_img.'" class="img-responsive pho-border"></a></div>';
		}
